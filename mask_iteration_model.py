...
from redress.display.plot_funcs import (open_sat_model, getdata, transpt, save_geotiff,
                         collocate_rasters, rmse,coordinates2xy)
import glob

sys.path.append("/home/nheilir/REDRESS/REDRESS/")
from redress.geospatial.gdal_ops import write_xarray_dset
...
outputs_folder = "/home/nheilir/REDRESS/REDRESS_files/outputs_iteration/"
inputs_folder =  "/home/nheilir/REDRESS/REDRESS_files/Entree_modele/"


# #  constant definition
#20180417 les valeurs sont unconues
measure_value = {'20180213' :{'aod':0.02 , 'ozone':0.3954 , 'water':0.175 , 'fixssa':41.41},
...
mask_bands=[2,4,5,8,9,10,11,12,15,16,17,18,19,21]
case=5
...
#     date = re.search(pattern, s3_str).group(1)
    ...
    osmd.model.set_rt_options("Urban", aod=measure_value[osmd.sat.date]['aod'],
                              refl=0, ozone=measure_value[osmd.sat.date]['ozone'],
                              water= measure_value[osmd.sat.date]['water'])
    
    
    for iterOptim in range(1,4):
